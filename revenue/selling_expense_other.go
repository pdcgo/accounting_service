...
// SellingExpenseOther implements revenue_ifaceconnect.RevenueServiceHandler.
func (r *revenueServiceImpl) SellingExpenseOther(ctx context.Context, req *connect.Request[revenue_iface.SellingExpenseOtherRequest]) (*connect.Response[revenue_iface.SellingExpenseOtherResponse], error) {
	var err error

	result := revenue_iface.SellingExpenseOtherResponse{}
	pay := req.Msg
	...
	err = accounting_core.OpenTransaction(ctx, db, func(tx *gorm.DB, bookmng accounting_core.BookManage) error {
		ref := accounting_core.NewStringRefID(&accounting_core.StringRefData{
			RefType: accounting_core.SellingExpenseOtherRef,
			ID:      pay.ExternalExpenseId,
		})
